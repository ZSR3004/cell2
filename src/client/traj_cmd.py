...
def traj(tune, default, flows):
    ...
    if stack_index < 0 or stack_index >= len(stacks):
        click.Exception("Invalid stack selection. Please run the command again and select a valid number.")
    ...
    if ftag.lower() == 'list' or ftag.lower() == 'l':
        ...
        raise click.Exception(f"File tag {ftag} not found in the flow directory. Please check the available tags.")
